...
* Results <br>
Contains the evaluation script that was modified by one of the master students in our lab and further modified by me to be able to run multiple cellprofiler images at once. Note that the **utils** folder must be placed in the same directory as the script before running it. for changing the false positive and false negative thresholds in the script: <br>
