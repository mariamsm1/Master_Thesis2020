...
y = Dense(1000, activation='relu', name = 'FC1')(y)
y = Dropout(0.5)(y)
...
hist = model.fit(x_train, y_train,epochs=12, validation_data=(x_val, y_val), callbacks = [csv_logger,checkpoint],verbose=1)
print("Done!")
...
n_epochs = 12
plt.figure()
plt.plot(np.arange(0, n_epochs), hist.history["loss"], label = "train_loss")
plt.plot(np.arange(0, n_epochs), hist.history["val_loss"], label = "val_loss")
plt.plot(np.arange(0, n_epochs), hist.history["accuracy"], label = "acc")
plt.plot(np.arange(0, n_epochs), hist.history["val_accuracy"], label = "val_acc")
